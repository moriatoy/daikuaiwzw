<template>
  <div class="productDetail">
    ...
    <div class="footer" @click="handleUrl(loanProduct.productUrl,loanProduct.productName,loanProduct.productId)">
      马上拿钱
    </div>
  </div>
</template>
...
import { getCredit,getLoanProductById,getUserInfo,channel,addRecord } from '../Ajax/post.js';
export default {
  name: 'productDetail',
  components: {
    Tab,
      ...
      dumbWrapper({
        ...
        successCB:(e)=>{
          if(e.state==0){
            ...
            let nameData = "productApply_"+id+"_"+e.data.userId+"_"+this.query;
            if (window.localStorage.getItem(nameData) !== timeData) {
              // 记录打开应用的UV的统计
              let indexData = this.$route.query.index;
              let data = {};
              if (indexData) {
                data = {
                  productName:name,
            ...
            // router.push({path:'/thirdPage',query:{url:productUrl,name:this.loanProduct.productName}});
