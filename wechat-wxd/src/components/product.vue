...
export default {
  ...
  methods:{
    ...
    handleUrl(url){
      ...
      if (type ===  "hot") {
        if (this.channel) {
          _czc.push(["_trackEvent","总量",self.loanProduct.productName,this.channel]);
          _czc.push(["_trackEvent","热门贷款",self.loanProduct.productName,this.channel]);
        } else {
          _czc.push(["_trackEvent","总量",self.loanProduct.productName]);
          _czc.push(["_trackEvent","热门贷款",self.loanProduct.productName]);
        }
      } else if (type === "log") {
        if (this.channel) {
          _czc.push(["_trackEvent","总量",self.loanProduct.productName,this.channel]);
          _czc.push(["_trackEvent","贷款大全",self.loanProduct.productName,this.channel]);
        } else {
          _czc.push(["_trackEvent","总量",self.loanProduct.productName]);
          _czc.push(["_trackEvent","贷款大全",self.loanProduct.productName]);
        }
      }
