...
public class GlideImageLoader implements IImageLoaderstrategy {
    ...
    public void showImage(@NonNull ImageLoaderOptions options) {
        ...
        if(mGenericRequestBuilder != null){
            showImageFinally(mGenericRequestBuilder,options);
        }
    }
    ...
    public GenericRequestBuilder init(ImageLoaderOptions options){
        ...
        if(v instanceof ImageView){
            ...
            //需要将图片进行相关transform    其强转成DrawableTypeRequestBuilder
            if(options.isBlurImage() || options.isCircleTransform()){
    ...
    private GenericRequestBuilder loadGenericParams(GenericRequestBuilder mGenericRequestBuilder, final ImageLoaderOptions options){
        ...
        if(mGenericRequestBuilder instanceof DrawableTypeRequest){
            if(options.isCenterCrop()){
                ((DrawableTypeRequest)mGenericRequestBuilder).centerCrop();
            }
        ...
        if (options.getImageSize() != null) {
            ...
            Log.i("test ","load params " + options.getImageSize().getWidth() + "  : " + options.getImageSize().getHeight());
    ...
    private void showImageFinally(GenericRequestBuilder mDrawableTypeRequest, ImageLoaderOptions options){
        final ImageView img = (ImageView) options.getViewContainer();

        //是否切成圆形
        if(options.isCircleTransform()){
            ((DrawableRequestBuilder)mDrawableTypeRequest)
        ...
        if(options.isBlurImage()){
            ((DrawableRequestBuilder)mDrawableTypeRequest)
                    .bitmapTransform(new BlurTransformation(img.getContext(),23,4))
                    .into(img);
            return;
        }
        //是否展示gif
        if(options.isAsGif()){
            ((GifRequestBuilder)mDrawableTypeRequest).dontAnimate()
